...
plans, calls its tools, and reproduces a matter-power-spectrum figure from a single query.
...
Two sub-agents (LLM roles): The **lead** plans first and writes
the report last; the **worker** executes one step per visit. ~300 lines of
Python in `agents/`:

| file  | job |
|---|---|
| `llm.py` | Gemini (default) or Groq via OpenAI-compatible LLM endpoints — `make_llm("groq")` |
| `mcp.py`  | load the server's tools as LangChain tools |
| `state.py`  | the shared state: task, plan, cursor, results, history |
| `nodes.py`  | the lead and worker roles |
| `graph.py`  | wire them up (+ `new_run` / `follow_up` helpers); the two `route_from_*` functions are the entire "supervisor" |
| `skills.py`  | named recipes in `skills/*.md`, loaded on demand via the `load_skill` tool |
| `memory.py`  | `MEMORY.md` lessons read at planning time; agents append via `remember` |
...
## What differs in a production version

Everything here scales up: several specialized workers (data / compute /
literature / visualization), checkpointing and resume, human-in-the-loop
approval before execution, internet access, and MCP servers
